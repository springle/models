...
def main(server, log_dir, context):
    # Set run_name for TensorBoard
    run_name = context.get("run_name") or datetime.datetime.now().strftime("%Y%m%d-%H%M%S")

    # Pass context variables to FLAGS
    FLAGS.master = server.target
    FLAGS.task = server.server_def.task_index
    FLAGS.is_chief = (server.server_def.task_index == 0)
    FLAGS.train_dir = log_dir + "/" + run_name + "/"
    FLAGS.model_name = context.get("model_name") or "inception_v3"
    FLAGS.dataset_dir = context.get("dataset_dir")
    FLAGS.dataset_name = context.get("dataset_name") or "cifar10"
    ...
    if len(server.server_def.cluster.job) > 1:
        ...
        FLAGS.num_ps_tasks = len(server.server_def.cluster.job[0].tasks)
        FLAGS.worker_replicas = len(server.server_def.cluster.job[1].tasks)
    else:
        # Not using parameter servers
        FLAGS.num_ps_tasks = 0
        FLAGS.worker_replicas = len(server.server_def.cluster.job[0].tasks)
